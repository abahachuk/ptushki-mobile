...
const PhotoResized = props => {
  ...
  return (
    <Overlay overlayBackgroundColor="rgba(0, 0, 0, 0.6)" fullScreen>
      <TouchableOpacity onPress={onCloseResizedPhoto} style={styles.resizedPhotoButton}>
        <Text style={styles.resizedPhotoButtonText}>{translate('addEditObservation.close')}</Text>
      </TouchableOpacity>
      <Image style={styles.birdPhotoResized} source={photoForResizing} />
      <TouchableOpacity onPress={onDeletePhoto} style={styles.resizedPhotoButton}>
        <Text style={styles.resizedPhotoButtonText}>{translate('addEditObservation.delete')}</Text>
      </TouchableOpacity>
    </Overlay>
  );
};
